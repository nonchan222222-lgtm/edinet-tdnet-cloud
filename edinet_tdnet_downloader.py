...
# 有報（ordinanceCode=010 & formCode=030000）、四半期：043000/043001
EDINET_FILTERS = {
...
def parse_codes_names(arg: Optional[str]) -> Tuple[Optional[set], Optional[List[re.Pattern]]]:
    ...
    for raw in arg.split(","):
        tok = raw.strip()
        if not tok: continue
        if CODE4.match(tok): sec_codes.add(tok)
        else: name_pats.append(re.compile(re.escape(tok), re.IGNORECASE))
...
def parse_args() -> Options:
    ...
    p.add_argument("--out", default="downloads", help="保存ルート。省略時は .env の OUTPUT_DIR を優先")
    p.add_argument("--edinet-filetypes", default="pdf", help="カンマ区切り: pdf,csv,xbrl")
    p.add_argument("--include-yuho", default="yes", help="有報 yes/no")
    p.add_argument("--include-quarter", default="no", help="四半期 yes/no")
    p.add_argument("--tdnet", default="yes", help="TDnet（短信） yes/no")
    p.add_argument("--tdnet-limit", type=int, default=1000)
    p.add_argument("--codes", default="", help="会社名または4桁コード（例: 7203,トヨタ）")
    ...
    for ft in filetypes:
        if ft not in EDINET_TYPE_MAP:
            raise SystemExit(f"不明なファイル種別: {ft}（pdf,csv,xbrl）")
    ...
    return Options(
        start=start, end=end, out=out_path, edinet_filetypes=filetypes,
        ...
        tdnet_limit=a.tdnet_limit, sec_codes=sec_codes, name_pats=name_pats,
        api_key=api_key, max_retries=a.max_retries, sleep_sec=a.sleep_sec,
    )
...
def edinet_pick(records: List[dict], include_yuho: bool, include_quarter: bool,
    ...
    for rec in records:
        ...
        ok_type = ((include_yuho and EDINET_FILTERS["yuho"](rec)) or
                   (include_quarter and EDINET_FILTERS["quarter"](rec)))
        if not ok_type: continue
...
def edinet_download(doc: dict, out_dir: Path, api_key: str, filetypes: List[str],
    ...
    saved: List[Path] = []
    doc_id = doc.get("docID"); issuer = doc.get("filerName") or ""; sec = str(doc.get("secCode") or "")
    for ft in filetypes:
...
def tdnet_entry_matches(e, sec_codes: Optional[set], name_pats: Optional[List[re.Pattern]]) -> bool:
    title = str(e.get("title","")); summary = str(e.get("summary",""))
    m = re.search(r"(?<!\d)(\d{4})(?!\d)", " ".join([title, summary])); code = m.group(1) if m else None
    if sec_codes and code and code in sec_codes: return True
    if name_pats:
        blob = title + " " + summary
        for pat in name_pats:
            if pat.search(blob): return True
...
def tdnet_download_dec_summary(start: dt.date, end: dt.date, out_root: Path,
    ...
    saved: List[Path] = []
    urls = tdnet_feed_urls(start, end, limit=1000); seen_links = set()
    for url in urls:
        ...
        for e in feed.entries:
            ...
            if not tdnet_entry_matches(e, sec_codes, name_pats): continue
            seen_links.add(link)
            try:
            ...
            pub_dt = None
            if e.get("published"):
                try: pub_dt = isoparse(e.published).date()
                except Exception: pub_dt = start
            day = pub_dt or start
    ...
    if opt.include_yuho or opt.include_quarter:
        ...
        for day in tqdm(list(daterange(opt.start, opt.end)), desc="EDINET list"):
            ...
            for rec in tqdm(picked, desc=f"EDINET dl {day}", leave=False):
                try:
                    paths = edinet_download(rec, day_dir, opt.api_key, opt.edinet_filetypes,
                                            opt.sleep_sec, opt.max_retries)
                except Exception as e:
            ...
            if rows: append_log(log_csv, rows)

    # TDnet（決算短信）
    if opt.tdnet:
        try:
            saved = tdnet_download_dec_summary(opt.start, opt.end, opt.out,
                                               opt.sec_codes, opt.name_pats, opt.sleep_sec)
        except Exception as e:
            print(f"[WARN] TDnet取得でエラー: {e}"); saved = []
        if saved:
            rows = [["TDNET", opt.start.isoformat()+"~"+opt.end.isoformat(), "", "", "", "決算短信",
